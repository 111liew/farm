...
const TRANSLATIONS = {
  ...
  // ... (Other languages kept brief for this version, normally would be fully translated)
   ms: {
...
const TaskCard = ({ task, onToggle, onDelete, onEdit }) => {
  const getIcon = (type) => {
    switch(type) {
      ...
      case 'fertilize': return <Sprout size={18} className="text-emerald-500" />;
      default: return <Check size={18} className="text-green-500" />;
    }
  };
  return (
    <div className="bg-white/60 backdrop-blur-md p-4 rounded-xl shadow-sm border border-white/50 flex items-center justify-between mb-3 transition-all hover:shadow-md hover:bg-white/70">
      <div className="flex items-center gap-3 cursor-pointer flex-1" onClick={() => onEdit && onEdit(task)}>
        <div className="w-10 h-10 rounded-lg bg-green-50 flex items-center justify-center">{getIcon(task.type)}</div>
        <div><h3 className={`font-medium text-gray-800 ${task.completed ? 'line-through text-gray-400' : ''}`}>{task.title}</h3><p className="text-xs text-green-600 flex items-center gap-1">📍 {task.location}</p></div>
      </div>
      <div className="flex flex-col items-end gap-2">
        {!task.completed && <span className="text-xs font-medium bg-green-50 text-green-700 px-2 py-0.5 rounded-full">{task.time}</span>}
        <div className="flex gap-2">
           {!task.completed && <button onClick={() => onToggle(task.id)} className="p-1.5 rounded-lg border border-gray-200 hover:bg-green-50 text-gray-400 hover:text-green-600 transition-colors"><Check size={16} /></button>}
           <button onClick={() => onDelete(task.id)} className="p-1.5 rounded-lg border border-gray-200 hover:bg-red-50 text-gray-400 hover:text-red-500 transition-colors"><Trash2 size={16} /></button>
        </div>
      </div>
    </div>
  );
...
export default function App() {
  ...
  const handleLogin = (e) => {
      e.preventDefault();
      // For Demo: Allow login if fields are filled
      if (userEmail && password && (isSignUp ? userName : true)) { 
          setIsLoggedIn(true);
          localStorage.setItem('farm_loggedin', 'true');
          // If login, try to get name from storage or default to 'User'
  ...
  if (showSplash) {
     return (
       <div className="min-h-screen bg-cover bg-center flex flex-col items-center justify-center p-6 relative overflow-hidden" 
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-green-900/60 via-transparent to-orange-400/30"></div>
          {/* Animated Particles (CSS Animation simulation) */}
          <div className="absolute top-0 left-0 w-full h-full animate-pulse opacity-30 pointer-events-none">
             <div className="absolute top-1/4 left-1/4 w-2 h-2 bg-yellow-200 rounded-full blur-sm"></div>
             <div className="absolute top-1/2 left-3/4 w-3 h-3 bg-white rounded-full blur-sm"></div>
          </div>
          
          <div className="relative z-10 text-center animate-in fade-in zoom-in duration-1000">
             <div className="w-24 h-24 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center mx-auto mb-6 shadow-2xl border border-white/40">
                 <Sprout size={48} className="text-white drop-shadow-md" />
             </div>
             <h1 className="text-4xl font-bold text-white tracking-tight drop-shadow-lg mb-2">Farm Manager</h1>
             <p className="text-white/90 font-medium text-sm tracking-wide">Cultivating the Future</p>
          </div>
       </div>
     )
  }
  ...
  if (!isLoggedIn) {
      return (
          <div className="min-h-screen bg-cover bg-center flex items-center justify-center p-4 relative" style={{ backgroundImage: "url('https://images.unsplash.com/photo-1500382017468-9049fed747ef?q=80&w=3200&auto=format&fit=crop')" }}>
              ...
              <div className="bg-white/90 backdrop-blur-xl p-8 rounded-[2rem] shadow-2xl w-full max-w-sm relative z-10 animate-in fade-in zoom-in-95 duration-500 border border-white/60">
